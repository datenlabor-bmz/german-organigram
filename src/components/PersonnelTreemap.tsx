...
import { getAllEntities, getEntityPersonalhaushalt } from '@/lib/entities';
import { Entity } from '@/types/entity';
import { useRouter } from 'next/navigation';

interface Rect { x: number; y: number; width: number; height: number; }
interface TreemapItem { value: number; entity: Entity; }
...
export default function PersonnelTreemap() {
    ...
    useEffect(() => {
        const entities = getAllEntities().filter(e => e.budgetMatch);
        const results = entities
            .map(entity => {
                const ph = getEntityPersonalhaushalt(entity.budgetMatch!);
                return { entity, value: ph?.zusammen_2025 || 0 };
            })
    ...
    if (loading) return <div className="w-full h-[calc(100vh-280px)] min-h-[700px] flex items-center justify-center"><p className="text-gray-500 text-lg">Lade Personaldaten...</p></div>;
    if (data.length === 0) return <div className="w-full h-[calc(100vh-280px)] min-h-[700px] flex items-center justify-center"><p className="text-gray-500 text-lg">Keine Personaldaten verfügbar</p></div>;

    const rects = squarify(data, 0, 0, 100, 100);
                    ...
                    return (
                        <Tooltip key={rect.entity.Organisation} delayDuration={50} disableHoverableContent>
                            <TooltipTrigger asChild>
                                <div
                                    className="absolute bg-emerald-400 text-gray-900 flex flex-col justify-center items-center p-2 transition-all duration-200 cursor-pointer"
                                    style={{
                                        left: `${rect.x}%`, top: `${rect.y}%`, width: `${rect.width}%`, height: `${rect.height}%`,
                                        opacity: isHovered ? 1 : 0.9, zIndex: isHovered ? 10 : 1, transform: isHovered ? 'scale(1.02)' : 'scale(1)',
                                    }}
                                ...
                                >
                                    {showLabel && (
                                        <>
                                            <div className="text-xs font-medium text-center mb-1 line-clamp-2 px-1">{displayName}</div>
                                            <div className="text-sm font-semibold">{rect.value.toLocaleString('de-DE')}</div>
                                        </>
                                    )}
                                </div>
                            </TooltipTrigger>
                            <TooltipContent side="top" sideOffset={8} className="bg-white text-slate-800 border border-slate-200 shadow-lg max-w-sm" hideWhenDetached avoidCollisions collisionPadding={12}>
                                <div className="text-center p-2">
                                    <p className="font-bold text-base mb-1">{rect.entity.Organisation}</p>
                                    {rect.entity.OrganisationKurz && <p className="text-xs text-gray-500 mb-2">{rect.entity.OrganisationKurz}</p>}
                                    <p className="text-lg font-semibold text-emerald-600">{rect.value.toLocaleString('de-DE')} Planstellen</p>
